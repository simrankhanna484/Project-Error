...
let fadeOut = false;
let fadeOpacity = 255;
let fadeDuration = 3000; // Time (in ms) before switching to a new canvas
let newCanvasInitiated = false;
const raycaster = new THREE.Raycaster();
const mouse = new THREE.Vector2();
...
function initiateNewCanvas() {
    ...
    if (glitchImage) {
        glitchImage.remove();  // Remove the video element
        glitchImage = null;    // Ensure it's set to null
    }

    let currentCanvas = document.querySelector('canvas');
    if (currentCanvas) {
        clear();  // Clear the old canvas before removing it
        currentCanvas.remove(); // Remove the old canvas
    }
...
function animate() {
    triangles.forEach(({ mesh, layer }) => {
        mesh.rotation.x += 0.005 * (layer + 1);
        mesh.rotation.y += 0.005 * (layer + 1);
        mesh.material.opacity = sin(frameCount * 0.05) * 0.5 + 0.5;
    });
    renderer.render(scene, camera);
    requestAnimationFrame(animate);
}
